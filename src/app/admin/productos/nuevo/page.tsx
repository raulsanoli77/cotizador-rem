...
export default function NuevoProducto() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const specsToSave = { ...especificaciones };
              ...
              {selectedCat.campos_filtro.map((campo: any) => (
                <div key={campo.nombre}>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    {(campo.sufijo || campo.unidad) && (
                      <span className="text-brand-600 font-mono ml-1">({campo.sufijo || campo.unidad})</span>
                    )}
                  </label>
                  ...
                  {campo.tipo === 'seleccion' && campo.opciones ? (
                    ...
                    <div className="relative">
                      <input 
                        ...
                        onChange={(e) => handleSpecChange(campo.nombre, e.target.value)}
                        className="w-full border-gray-300 rounded-md focus:ring-brand-500 focus:border-brand-500 sm:text-sm"
                        placeholder={campo.sufijo ? `Ej. 30${campo.sufijo}` : ''}
                      />
                      {(campo.sufijo || campo.unidad) && (
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 font-mono text-sm pointer-events-none">
                          {campo.sufijo || campo.unidad}
                        </span>
                      )}
                    </div>
